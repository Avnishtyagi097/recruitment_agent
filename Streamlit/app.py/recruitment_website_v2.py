...
defaults = {
    ...
    "users_db": {
        ...
        "recruiter": {"password_hash": hashlib.sha256("recruit123".encode()).hexdigest(), "name": "Sarah Johnson", "role": "recruiter"},
        "demo": {"password_hash": hashlib.sha256("demo123".encode()).hexdigest(), "name": "Demo User", "role": "recruiter"},
    },
    # Theme
    "theme": "dark",
    "show_landing": True,
...
def render_login_page():
    ...
    with col2:
        ...
        with login_tab:
            ...
            if st.button("\U0001f680 Sign In", type="primary", use_container_width=True, key="login_btn"):
                if username and password:
                    ...
                    if user and user["password_hash"] == pw_hash:
                        st.session_state.authenticated = True
                        st.session_state.current_user = {"username": username, "name": user["name"], "role": user["role"]}
                        st.session_state.show_landing = False
            ...
            st.code("admin / admin123\nrecruiter / recruit123\ndemo / demo123", language="text")

        with signup_tab:
            new_user = st.text_input("Choose Username", placeholder="e.g., johndoe", key="signup_user")
            new_name = st.text_input("Full Name", placeholder="e.g., John Doe", key="signup_name")
            new_pass = st.text_input("Password", type="password", placeholder="Min 6 characters", key="signup_pass")
            new_pass2 = st.text_input("Confirm Password", type="password", placeholder="Re-enter password", key="signup_pass2")
...
def render_landing_page():
    ...
    <div class="landing-hero animate-in">
        <div style="position:relative; z-index:1;">
            ...
            <h1>Talent Hunt<br></h1>
            <p>Automated candidate screening, assessments, and communications with our intelligent
            recruitment agent. Save Time, Reduce Bias & Hire The Best Talent Faster.</p>
...
def render_analytics_page():
    ...
    with col2:
        ...
        assess_sent = sum(1 for c in candidates.values() if c.get("status") in ["Assessment Sent", "Passed Assessment", "Failed Assessment", "Interview Scheduled"])
        passed_assess = sum(1 for c in candidates.values() if c.get("assessment_result", {}).get("decision") == "PASS")
    ...
    with col3:
        ...
        for c in candidates.values():
            all_missing.extend(c.get("ats_result", {}).get("missing_skills", []))
        if all_missing:
    ...
    with col5:
        ...
        for c in candidates.values():
            bd = c.get("ats_result", {}).get("score_breakdown", {})
            if bd:
...
# Gate 3: Authenticated - Full App
    ...
    if user:
        ...
        <div style="background:rgba(99,102,241,0.15); border-radius:12px; padding:0.8rem 1rem; margin:0.5rem 0;">
            ...
            <div style="font-size:0.75rem; color:#94A3B8 !important;">{user['role'].title()} \u2022 @{user['username']}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if page == "\U0001f4c4 CV Upload & ATS":
        st.markdown("""
        <div class="hero-banner animate-in">
            <h1>\U0001f4c4 CV Upload & ATS Screening</h1>
            <p>Upload a candidate's CV, analyze it against the job description, and let AI handle the rest.</p>
        </div>
        """, unsafe_allow_html=True)

        col_left, col_right = st.columns([1, 1], gap="large")
        with col_left:
            st.markdown('<div class="section-card"><h3>\U0001f464 Candidate Information</h3>', unsafe_allow_html=True)
        ...
        with col_right:
            st.markdown('<div class="section-card"><h3>\U0001f4cb Job Description</h3>', unsafe_allow_html=True)
        ...
        if st.button("\U0001f50d Analyze CV & Generate ATS Score", type="primary", use_container_width=True):
            if not c_name: st.error("\u274c Please enter the candidate's name.")
                ...
                if not cv_text: st.error("\u274c No CV content found.")
                    ...
                    with st.spinner("\U0001f504 Analyzing CV..."):
                    ...
                    if dec == "PASS":
                        st.markdown(f'<div class="result-banner pass-banner">\U0001f389 {c_name} PASSED — Score: {score}%</div>', unsafe_allow_html=True)
                    ...
                    with st.expander("\U0001f527 Full ATS JSON"):
                    ...
                    if dec == "PASS":
                        ...
                        st.info("\U0001f449 Navigate to **\U0001f4dd Assessment** page to proceed.")
                    elif result["requires_human_review"]:
    ...
    # ═════════════════════════════════════════════
        ...
        if not candidates: st.info("No candidates yet.")
            ...
            total = len(candidates)
            p_ats = sum(1 for c in candidates.values() if c.get("ats_result", {}).get("decision") == "PASS")
            p_ass = sum(1 for c in candidates.values() if c.get("assessment_result", {}).get("decision") == "PASS" if c.get("assessment_result"))
