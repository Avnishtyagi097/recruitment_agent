...
page = st.sidebar.radio(
    ...
    ["📄 CV Upload & ATS", "📝 Assessment", "📊 Results & Interview",
     "📋 Pipeline Logs", "👤 Candidate Dashboard"],
    index=0,
)
...
if st.session_state.current_candidate_id and st.session_state.current_candidate_id in st.session_state.candidates:
    ...
    status_color = {"Passed ATS": "🟢", "Failed ATS": "🔴", "Assessment Sent": "🟡",
                    "Passed Assessment": "🟢", "Failed Assessment": "🔴",
                    "Interview Scheduled": "🟢", "Manual Review": "🟠"}.get(status, "⚪")
...
if page == "📄 CV Upload & ATS":
    st.markdown("# 📄 CV Upload & ATS Screening")
    st.markdown("Upload a candidate's CV and analyze it against the job description to generate an ATS match score.")
    st.markdown("---")
    ...
    with col_right:
        ...
        with st.expander("ℹ️ Scoring Methodology"):
            st.markdown("""
            The ATS score is calculated using weighted criteria:
            | Component | Weight |
            |---|---|
            | Skill Match | 40% |
            | Experience | 20% |
    ...
    if st.button("🔍 Analyze CV & Generate ATS Score", type="primary", use_container_width=True):
        if not c_name:
            ...
            if not cv_text:
                ...
                st.markdown(f"**📅 Experience Match:** {result.get('experience_match', 'N/A')}")

                with st.expander("🔧 Full ATS JSON Output"):
                    json_output = {
                ...
                if result["decision"] == "PASS":
                    st.success(f"🎉 **{c_name}** has PASSED the ATS screening with a score of **{result['ats_score']}%**!")
                    add_log(c_id, "ATS_SCREENING", "PASS", result["ats_score"], result["reasoning_summary"], "Send Assessment", "AI_AGENT")
                    deadline = (datetime.now() + timedelta(days=2)).strftime("%B %d, %Y at 11:59 PM UTC")
                    email = generate_assessment_email(c_name, role_applied, deadline, st.session_state.recruiter_name)
                    candidate_data["emails_sent"].append({"type": "Assessment Invitation", "content": email, "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                    candidate_data["status"] = "Assessment Sent"
                    st.session_state.candidates[c_id] = candidate_data
                    with st.expander("📧 Assessment Invitation Email Preview", expanded=True):
                        st.code(email, language="text")
                    st.info("👉 Navigate to **📝 Assessment** page to proceed with the assessment.")
                    ...
                    add_log(c_id, "ATS_SCREENING", "FAIL", result["ats_score"], result["reasoning_summary"], "Send Rejection Email", "AI_AGENT")
                    email = generate_rejection_email(c_name, role_applied, "ATS", st.session_state.recruiter_name)
                    candidate_data["emails_sent"].append({"type": "ATS Rejection", "content": email, "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                    st.session_state.candidates[c_id] = candidate_data
                    with st.expander("📧 Rejection Email Preview", expanded=True):
                        st.code(email, language="text")
        ...
        if status in ["Failed ATS"]:
            ...
            with st.expander("\U0001f4cb Assessment Instructions & Anti-Cheating Notice", expanded=not st.session_state.assessment_started):
                ...
                **\u26a0\ufe0f Anti-Cheating Policy:**
                ...
                - Auto-rejection based solely on a technical signal is NOT applied; flagged cases are reviewed by a human recruiter.
                ...
                if not questions:
                    ...
                    if st.button("\u2705 Submit Assessment", type="primary", use_container_width=True):
                        if answered < len(questions):
                            st.warning(f"\u26a0\ufe0f You have answered {answered}/{len(questions)} questions. Unanswered questions will be marked incorrect.")
                        ...
                        cand["assessment_result"]["decision"] = decision
                        ...
                        add_log(cid, "ASSESSMENT", decision, result["score_percent"],
                                ...
                                "Move to Interview" if decision == "PASS" else "Send Rejection Email", "AI_AGENT")
                        st.rerun()
                else:
                    ...
                    with st.expander("\U0001f527 Full Assessment JSON Output"):
                        json_out = {
                            "candidate_id": cid, "stage": "ASSESSMENT",
                            "assessment_score_percent": result.get("score_percent", 0),
                            "decision": decision, "role": cand["role"],
                            "strength_areas": result.get("strength_areas", []),
                            "weak_areas": result.get("weak_areas", []),
                            "cheating_flag": False,
                            "recommended_next_action": "Move to Interview" if decision == "PASS" else "Send Rejection Email",
                        }
                        st.json(json_out)

                    st.info("\U0001f449 Navigate to **\U0001f4ca Results & Interview** to see next steps.")
    ...
    if not cid or cid not in st.session_state.candidates:
        ...
        if assessment:
            ...
            if assessment.get("decision") == "PASS":
                st.success("\U0001f389 Candidate has cleared the assessment! Proceeding to interview scheduling.")
                st.markdown("---")
                ...
                if slots:
                    ...
                    slot_df["panel"] = slot_df["panel"].apply(lambda x: ", ".join(x))
                    st.dataframe(slot_df, use_container_width=True, hide_index=True)

                    if st.button("\U0001f4e7 Send Interview Invite Email", type="primary", use_container_width=True):
                        email = generate_interview_email(cand["name"], cand["role"], slots, st.session_state.recruiter_name)
                        cand["emails_sent"].append({"type": "Interview Invitation", "content": email, "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                        cand["status"] = "Interview Scheduled"
                        cand["interview_scheduled"] = True
                        st.session_state.candidates[cid] = cand
                        add_log(cid, "INTERVIEW_SCHEDULING", "SCHEDULED", "N/A",
                                f"Interview invite sent with {len(slots)} slot options",
                                "Await candidate slot selection", "AI_AGENT")
                        st.success("\u2705 Interview invitation email sent successfully!")
                        with st.expander("\U0001f4e7 Interview Invite Email Preview", expanded=True):
                            st.code(email, language="text")
                else:
                    st.error("\u274c No available slots found in the next 3 days. Escalating to recruiter.")
                    add_log(cid, "INTERVIEW_SCHEDULING", "ESCALATED", "N/A",
                            "No panel availability in next 3 days", "Notify recruiter manually", "AI_AGENT")

                if cand.get("interview_scheduled"):
                    st.markdown("---")
                    st.success("\u2705 Interview has been scheduled. Status updated.")

            elif assessment.get("decision") == "FAIL":
                st.error(f"\u274c Candidate scored **{assessment.get('score_percent', 0)}%** and did not pass the assessment (threshold: >90%).")
                email = generate_rejection_email(cand["name"], cand["role"], "Assessment", st.session_state.recruiter_name)
                if not any(e["type"] == "Assessment Rejection" for e in cand.get("emails_sent", [])):
                    cand["emails_sent"].append({"type": "Assessment Rejection", "content": email, "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                    st.session_state.candidates[cid] = cand
                with st.expander("\U0001f4e7 Rejection Email Preview", expanded=True):
                    st.code(email, language="text")
        else:
            st.info("\u2139\ufe0f Assessment not yet completed. Please complete the assessment on the **\U0001f4dd Assessment** page.")
        ...
        if emails:
            ...
            for i, em in enumerate(emails):
                with st.expander(f"\U0001f4e7 {em['type']} \u2014 {em['timestamp']}"):
...
# ═════════════════════════════════════════════
    ...
    if not logs:
        ...
        for log in filtered:
            icon = {"PASS": "\U0001f7e2", "FAIL": "\U0001f534", "REVIEW": "\U0001f7e0", "IN_PROGRESS": "\U0001f535",
                    "SCHEDULED": "\U0001f7e2", "ESCALATED": "\U0001f7e1"}.get(log["decision"], "\u26aa")
...
# ═════════════════════════════════════════════
    ...
    if not candidates:
        ...
        for cid_key, cand_val in candidates.items():
            ...
            status_icon = {"Passed ATS": "\U0001f7e2", "Failed ATS": "\U0001f534", "Assessment Sent": "\U0001f7e1",
                           "Passed Assessment": "\U0001f7e2", "Failed Assessment": "\U0001f534",
                           "Interview Scheduled": "\u2705", "Manual Review": "\U0001f7e0"}.get(status, "\u26aa")
